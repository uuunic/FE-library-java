...
public class SS_CCA_HIBBE {
	...
	public Element Decrypt(HIBBEpp pp, HIBBEct ct, HIBBEsk sk){
		if (!BB04.Verification(pairing, ct.get_vk(), ct.get_sign())){
			return null;
		}
		...
		byte[] byte_vk = ct.get_vk().getBytes();
		Element hash_vk = GroupHash.HashToZp(pp.get_pairing(), byte_vk);
		K = K.mul(sk.get_b(pp.get_max_role()).powZn(hash_vk));
